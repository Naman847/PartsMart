...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($report_type === 'product_stock') {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            // Updated stock status logic to cover all cases
            if ($qty < $moq) {
                $status = 'OUT OF STOCK';
            } elseif ($qty >= $moq && $qty <= $moq * 5) {
                $status = 'NORMAL STOCK';
    ...
    if (isset($_POST['download']) && !empty($report_data)) {
        ...
        $pdf->Cell(0, 7, 'Inventory Report', 0, 1, 'L');
        $pdf->SetFont('Arial', '', 12);
        
        // Underlined report title
        $title = ($report_type === 'product_stock' ? 'Product Stock Report' : 'Stock Level Report ') . ' : ';
        $pdf->Cell($pdf->GetStringWidth($title), 10, $title, 0, 0, 'L');
        
        $reportTitle = $report_type === 'product_stock' ? strtoupper($selected_product_name) : strtoupper($stock_level);
        $pdf->SetFont('Arial', 'U', 12); // Underlined
        $pdf->Cell($pdf->GetStringWidth($reportTitle), 10, $reportTitle, 0, 1, 'L');
        ...
        <form method="POST" onsubmit="return validateForm()">
            ...
            <div class="form-group" id="stock_level_group" style="display: none;">
                ...
                <select id="stock_level" name="stock_level">
                    ...
                    <option value="NORMAL STOCK" <?php echo (isset($_POST['stock_level']) && $_POST['stock_level'] == 'NORMAL STOCK') ? 'selected' : ''; ?>>NORMAL STOCK</option>
