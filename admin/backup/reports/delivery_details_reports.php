...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (isset($_POST['download']) && !empty($report_data)) {
    ...
    $pdf->SetTextColor(0, 0, 0);
    $pdf->Cell(0, 10, 'Delivery Details Report', 0, 1, 'L');
    $pdf->SetFont('Arial', '', 12);
    $title = "Delivery Details Report: $start_date to $end_date";
    if ($selected_agent !== 'all') {
...
<head>
  ...
  <style>
    ...
    table {
      ...
      margin: 15px 0 30px 0; /* Increased bottom margin for spacing between tables */
      table-layout: fixed;
      font-size: 12px;
    }
